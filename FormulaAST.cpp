...
public:
    ...
    double operator()(const std::string &value) {
        ...
        // иначе пробуем преобразовать в число (если получится то возвращаем число, если нет - кидаем ошибку)
        try {
            size_t pos;
            auto res = std::stod(value, &pos);
            if (pos == value.size()) {
                return res;
            } else {
    ...
    double operator()(FormulaError err) {
        throw err;
    }
};

double ValueToDouble(const CellInterface::Value &value) {
...
public:
    ...
    double Evaluate(const SheetInterface &sheet) const override {
        ...
        if (auto cell = sheet.GetCell(*cell_)) {
            return ValueToDouble(cell->GetValue());
        } else {
            return 0;
        }
    }
